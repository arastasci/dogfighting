...
void at::Model::LoadModel(const std::string& path)
{
	...
	if (!scene->HasMeshes())
	{
		AT_CORE_INFO("Error importing model at path {0}: {1}", path, "The model doesn't have any mesh.");
		return;
	}
...
std::vector<at::Texture> at::Model::LoadMaterialTextures(aiMaterial* mat, aiTextureType type, std::string typeName)
{
	...
	for (unsigned int i = 0; i < mat->GetTextureCount(type); i++)
	{
		...
		{   // if texture hasn't been loaded already, load it
			Texture texture;
			texture.id = TextureFromFile(str.C_Str(), this->m_Directory, true);
